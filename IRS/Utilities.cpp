...
namespace Csv {
void ToFile(const std::string& filename, const std::vector<std::vector<std::string>>& data, const std::string& separator, bool transpose, std::string& errormessage, bool append)
{
  errormessage = "";
  if (data.size() == 0)
  {
  ...
  if (append)
  {
    ...
    if (transpose) //must be same columns!
    {
      ...
      {
        for (int i = 0; i < static_cast<int>(data.size()) - 1; i++) { outputfile << (data[i][j]); outputfile << separator; }
    ...
    else
    {
      ...
      {
        for (int j = 0; j < static_cast<int>(data[i].size()) - 1; j++) { outputfile << (data[i][j]); outputfile << separator; }
...
void ToFile(const std::string& fileName, const std::map<std::string, std::vector<double>>& data, const std::string& seperator) {
  ...
  std::vector<std::vector<std::string>> outputContent(data.size(), std::vector<std::string>(data.begin()->second.size() + 1));
