...
public:
  Greek() = default;
  explicit Greek(Step h, Order order, const FiniteDifferenceMethod method = FiniteDifferenceMethod::Forward) : h_(h), method_(method), order_(order)
  {
  ...
  Step GetStepSize() const {
    return h_;
  }

  void SetStepSize(Step h) {
    h_ = h;
  }

  FiniteDifferenceMethod GetMethod() const {
    return method_;
  }
...
public:
  explicit DV01(Step h, const FiniteDifferenceMethod method = FiniteDifferenceMethod::Forward) : Greek(h, 1, method) {};
...
public:
  explicit Convexity(Step h, const FiniteDifferenceMethod method = FiniteDifferenceMethod::Forward) : Greek(h, 2, method) {};
